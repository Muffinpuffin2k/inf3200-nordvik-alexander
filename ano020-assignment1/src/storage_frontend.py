...
class StorageServerFrontend:
	...
	def __init__(self):
		#self.map = dict()		#  Dictionary which holds the key/value pairs
		self.size = 0
	...
	def sendPUT(self, key, value, size):
		print "first length:", len(storageBackendNodes)
...
class StorageServerTest:
	...
	def getTestObject(self, key, value):
		...
		try:
			conn = httplib.HTTPConnection(node, node_port)
